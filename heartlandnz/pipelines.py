...
class HheartlandnzPipeline:

    # Database setup
    conn = sqlite3.connect('heartlandnz.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `heartlandnz`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM heartlandnz WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO heartlandnz (date, title, link, content)"
